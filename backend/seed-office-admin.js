/**
 * seed-office-admin.js
 * Creates the Test Office Admin account in BOTH Firebase Auth AND MongoDB.
 *
...
// ─── Firebase Config (same as frontend / seed-firebase-users.js) ───
...
const TARGET = {
  ...
  email: 'officeadmin@test.com',
  password: 'Admin@1234',
  role: 'office_admin',
};

async function seed() {
  console.log('\n🔐 Seeding Test Office Admin account…\n');

  // 1. Initialize Firebase Client SDK
  const app = initializeApp(firebaseConfig, 'office-admin-seeder');
  ...
  if (existing) {
    ...
    if (firebaseUid && existing.firebaseUid !== firebaseUid) {
      ...
      await existing.save();
      console.log('   📝 firebaseUid synced to MongoDB');
    } else {
  ...
  // ── Summary ──────────────────────────────────────────────────────────────
  console.log('\n╔══════════════════════════════════════════╗');
  console.log('║     ✅ Office Admin Account Ready        ║');
  console.log('╠══════════════════════════════════════════╣');

...
if (!file) {
  throw new Error('Usage: node scripts/import-global-vehicle-colors.js --file=colors.ndjson [--apply] [--batch-size=500]');
}

async function* readRecords(path) {
  if (path.toLowerCase().endsWith('.json')) {
    const parsed = JSON.parse(await readFile(path, 'utf8'));
    if (!Array.isArray(parsed)) throw new Error('A .json color catalog must contain an array. Use NDJSON for large files.');
...
try {
  for await (const record of readRecords(file)) {
    normalizeVehicleColorRecord(record);
    count += 1;
    if (!apply) continue;
    batch.push(record);
    if (batch.length >= batchSize) {
      await upsertVehicleColorBatch(batch, { maxBatchSize: batchSize });
      batch = [];
      console.log(`Imported ${count} vehicle color records.`);
    }
  }
  if (apply && batch.length) await upsertVehicleColorBatch(batch, { maxBatchSize: batchSize });
  console.log(`${apply ? 'Imported' : 'Validated'} ${count} vehicle color records.`);
  if (apply) console.log(JSON.stringify(await vehicleColorCoverage(), null, 2));

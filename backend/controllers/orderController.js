...
export const getAllOrders = async (req, res, next) => {
  try {
    ...
    // Enforce customer role restriction
...
export const createOrder = async (req, res, next) => {
  try {
    ...
    const resolvedCustomerId =
      req.user.role === 'admin' && customerInput && mongoose.Types.ObjectId.isValid(customerInput)
        ? customerInput
        : req.user.id;
    ...
    if (req.user.role === 'customer') {
      const detailers = await User.find({ role: 'detailer', isActive: true }).select('_id name');
...
export const signWaiver = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== 'admin' && (!order.customer || order.customer.toString() !== req.user.id)) {
...
export const updateInspection = async (req, res, next) => {
  try {
    ...
    const invalidPhoto = preServicePhotos.find((url) => !isSignedUrl(url));
    if (invalidPhoto) {
      return res.status(400).json({
        success: false,
        message: 'Pre-service photos must be signed URLs from a private bucket'
      });
    }
  ...
  try {
    ...
    if (previousStatus !== order.status) {
      ...
      try {
        ...
        console.error('Failed to log order status change:', logError.message);
      }
    }
...
export const assignDetailerAndMarkPaid = async (req, res, next) => {
  try {
    ...
    // Defensive guard – route already uses authorize('admin')
    if (!req.user || req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        message: 'Only admins can assign detailers and mark payments as paid',
      });
    }
    ...
    if (previousStatus !== order.status) {
      try {
        ...
        console.error('Failed to log unified order update:', logError.message);
      }
    }
    ...
    try {
      ...
      if (previousStatus !== order.status) {
        await ActivityLog.create({
          ...
          metadata: {
            ...
            previousStatus,
            newStatus: order.status,
            vehicleInfo: `${order.vehicleYear} ${order.vehicleMake} ${order.vehicleModel}`.trim()
          }
        });
      }
      ...
      if (previousStatus === 'assigned' && order.status === 'in-progress') {
        await ActivityLog.create({
          type: 'started_job',
          title: 'Job Started',
          description: `Started job ${order.orderNumber}`,
          userId: req.user.id,
          userName: req.user.name || 'Detailer',
          metadata: {
            orderId: order._id,
            orderNumber: order.orderNumber,
            vehicleInfo: `${order.vehicleYear} ${order.vehicleMake} ${order.vehicleModel}`.trim()
      ...
      if (order.status === 'completed' && previousStatus !== 'completed') {
        await ActivityLog.create({
          type: 'completed_job',
          title: 'Job Completed',
          description: `Completed job ${order.orderNumber}`,
          userId: req.user.id,
          userName: req.user.name || 'Detailer',
          metadata: {
            orderId: order._id,
    ...
    try {
      if (previousCustomerStatus !== normalizedStatus) {
        ...
        await ActivityLog.create({
          ...
          description: `Customer status changed from ${previousCustomerStatus} to ${normalizedStatus}`,
          userId: req.user.id,
          userName: req.user.name || 'Staff',
          metadata: {
            orderId: order._id,
            orderNumber: order.orderNumber,
            previousStatus: previousCustomerStatus,
            newStatus: normalizedStatus,
          },
        });
      }
    } catch (logError) {
      console.error('Failed to log customer status change:', logError.message);
    }

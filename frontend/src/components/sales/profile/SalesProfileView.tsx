...
import AdminEditPersonalInfoModal, {
  ...
  type SocialLinks,
} from '@/components/Administrator/AdminEditPersonalInfoModal';
import AdminEditAddressModal, { type AddressDraft } from '@/components/Administrator/AdminEditAddressModal';
import AdminPasswordInput from '@/components/Administrator/AdminPasswordInput';
import '@/components/Administrator/administrator.css';
...
export default function SalesProfileView({ onNavigateHome }: Props) {
  ...
  const roleLabel = getRoleLabel(getSafeUserRole(user?.role));
  const profileUserId = String(user?.id || user?._id || '');
  const lastSavedAddressRef = useRef<string | null>(null);

  useEffect(() => {
    lastSavedAddressRef.current = null;
    setProfileStorage(parseProfileStorage(user?.address, roleLabel));
  }, [profileUserId]);

  useEffect(() => {
    const serialized = user?.address ?? '';
    if (serialized && serialized === lastSavedAddressRef.current) return;
    setProfileStorage(parseProfileStorage(user?.address, roleLabel));
  }, [user?.address, roleLabel]);

  const displayName = user?.name?.trim() || 'Signed in';
  const heroAvatar = useMemo(
    () => resolveProfileAvatar(profileUserId, avatarDraft ?? user?.avatar),
    [avatarDraft, profileUserId, user?.avatar],
  );
  const locationLabel =
  ...
  }, [profileStorage.bio, roleLabel, user?.email, user?.name, user?.phone]);
  ...
  const openProfileModal = () => {
    setPersonalDraft(buildPersonalDraft(user, roleLabel, profileStorage));
  ...
  const persistProfilePatch = async (
    ...
    const syncResult = await updateUser({
      ...
      id: profileUserId,
      _id: profileUserId,
      name: String(data.name || patch.name || user?.name || ''),
      email: String(data.email || patch.email || user?.email || ''),
      phone: typeof data.phone === 'string' ? data.phone : patch.phone || user?.phone,
      avatar:
  ...
  const saveProfile = async () => {
    ...
    const email = personalDraft.email.trim() || user?.email || '';
    if (!email) {
      ...
      const result = await persistProfilePatch(
        {
          ...
          avatar: avatarDraft ?? user?.avatar ?? null,
          address: serializedAddress,
        },
        nextStorage,
      );
  ...
  return (
    <div className="adminhub-root" style={{ display: 'block', height: 'auto', minHeight: 'auto', background: 'transparent' }}>

...
function validateSchedule(value: unknown): PublicScheduleDay[] {
    if (!Array.isArray(value) || value.length !== 7) {
        throw new Error('Public availability schedule is incomplete.');
    }

    const byDay = new Map<number, PublicScheduleDay>();
    for (const item of value) {
        if (!item || typeof item !== 'object') throw new Error('Invalid availability row.');
        const row = item as Partial<PublicScheduleDay>;
        if (!Number.isInteger(row.dow) || Number(row.dow) < 0 || Number(row.dow) > 6 || byDay.has(Number(row.dow))) {
            throw new Error('Invalid availability day.');
        }
        if (typeof row.open !== 'boolean') throw new Error('Invalid availability state.');
    ...
    return DAY_ORDER.map((dow) => {
        ...
        if (!row) throw new Error('Public availability schedule is incomplete.');
        return row;
    });
}
...
function buildDisplayRows(days: PublicScheduleDay[], locale: PublicScheduleLocale): PublicScheduleDisplayRow[] {
    ...
    for (const day of days) {
        ...
        const previousDay = previous?.[0];
        const sameHours = previousDay
            && previousDay.open === day.open
            && (!day.open || (previousDay.from === day.from && previousDay.to === day.to));
    ...
    return groups.map((group) => {
        ...
        const day = group.length === 1 ? firstName : `${firstName} – ${lastName}`;
        return {
            day,
            open: first.open,
            time: first.open
...
export function usePublicAvailabilitySchedule(locale: PublicScheduleLocale = 'en') {
    ...
    useEffect(() => {
        ...
        api.get<{ success: boolean; data: unknown }>('/slots/schedule', {
            signal: controller.signal,
            meta: { suppressErrorToast: true, suppressCancelLog: true },
        } as any)
            .then(({ data }) => {
                if (!data?.success) throw new Error('Public availability schedule request failed.');
                setState({ status: 'ready', days: validateSchedule(data.data) });
            })
            .catch((error) => {
                if (controller.signal.aborted) return;
                console.warn('Could not load public appointment hours.', error);
                // Marketing pages must not fall back to made-up hours. A generic
                // message is safer than advertising a schedule that may be closed.

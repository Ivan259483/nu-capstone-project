 ...
 *
 * Keep in sync with mobile/src/utils/customer-tracker-stage.ts.
 */

import { isForwardTrackerStageTransition, normTrackerStr } from './customer-live-tracker-pick.ts';
 ...
 * Quality Check. Delegates to the shared tracker-rank guard so there is a single
...
export function isForwardCustomerStageTransition(
  ...
  return isForwardTrackerStageTransition(current as any, incoming);
}
